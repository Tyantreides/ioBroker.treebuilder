...
const TreeItem = ({items, item, classes}) => {
    ...
    const renderRow = (items, item) => {
        let children = items.filter(i => i.native.parentId === item.native.id);
        let isExpanded = true;
        ...
        let iconStyle = {};
        let countSpan = children.length ? <span className={classes.childrenCount}>{children.length}</span> : null;
        ...
        const itemRow =
            <TableRow key={item.native.id} className={Utils.clsx(classes.hoverRow, treeBuilderContext.state.tree.selectedItem === item.native.id && classes.selected)} padding="default" style={{ ...style, background }} onClick={e => onSelect(item.native.id)}>
            <TableCell
                id={'td_' + item._id}
                colSpan={3}
                size="small"
