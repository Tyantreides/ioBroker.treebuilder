...
import { ListItemText } from '@material-ui/core';
 ...
 const TreeElementDialog = ({item = false, closeCallback = false, saveCallback = false, classes}) => {
    ...
    const [itemState, changeItemState] = useState(item ? item : getNewItem());
    ...
    const onClose = () => {
        treeBuilderContext.changeState({type: globalActions.SET.DIALOG.TREEELEMENTS.VISIBLE, payload: false})
    ...
    const onSave = async () => {
            let item = {...itemState}
            if (treeBuilderContext.state.dialog.action === dialogActions.new) {
                ...
                treeBuilderContext.iobData.createObject(item);
    ...
    return (
        <>
         {treeBuilderContext.state.dialog.treeElements.visible ? <Dialog
             open={true}
             maxWidth="md"
             fullWidth={true}
             onClose={() => { onClose() }}
 ...
 export default withStyles(styles)(TreeElementDialog);